...
class RemoteAudioService:
            ...
            for i, device in enumerate(devices):
                ...
                if device['max_output_channels'] > 0:
                    ...
                    if is_virtual_audio:
                        ...
                        print(f"   🎤➡️  {i:2d}: {device_name} (播放目标 - {device['default_samplerate']:.0f} Hz)")
                    else:
    ...
    def _handle_set_device(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if device_id is not None:
            if self.is_valid_device_id(device_id):
                ...
                print(f"🎛️  设备已更改: {old_device_name} -> {new_device_name} (ID: {device_id})")
    ...
    def _play_audio_to_virtual_microphone(self, temp_audio_path: str) -> bool:
        ...
        try:
            ...
            # 播放音频并等待完成
            try:
                print(f"▶️  开始播放音频 (采样率: {sample_rate} Hz)...")
